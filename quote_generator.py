...
from model_provider import get_model_provider, get_default_model, ProviderError
...
for persona in PERSONA_OPTIONS:
    ...
    first_char = short_name[0] if short_name else "🎤"
    CHARACTER_ICONS[persona] = first_char
    ...
    return [
        {
            ...
            "short_name": persona.split("(")[0].strip(),
            "avatar_path": CHARACTER_AVATARS.get(persona, None),
        }
        for persona in PERSONA_OPTIONS
    ]
...
"ai",
"generate_quote",
details=f"provider={provider_name} model={model} persona={persona}",
):
...
short_name = persona.split("(")[0].strip()
raise RuntimeError(
f"Failed to generate response using {provider_name} provider (model={model}): {exc}"
) from exc
except Exception as exc:
short_name = persona.split("(")[0].strip()
raise RuntimeError(
f"Could not generate {persona}: {exc}"
) from exc
